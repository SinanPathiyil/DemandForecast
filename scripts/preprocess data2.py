...
# Read your CSV file (skip header rows)
file_path = r'C:\Users\Strix\Desktop\Boehm Tech\demand forecasting\data\daily sale.csv'
...
# Select only first 3 columns to avoid extra unnamed columns
df = df.iloc[:, :3]
df.columns = ['Particulars', 'Date', 'Qty']
...
df['Particulars'] = df['Particulars'].str.replace('*', '', regex=False).str.strip()
...
# Remove 2020 data (incomplete year)
df = df[df['Year'] != 2020]

# Create Year-Week identifier (e.g., "2021-W01")
...
# Get global min and max year across all data
global_min_year = int(df_weekly['Year'].min())
global_max_year = int(df_weekly['Year'].max())

print(f"\nData range: {global_min_year} to {global_max_year} (capped at 2025-W42)")
...
for product in products:
    ...
    # Generate all year-week combinations from global min year to 2025-W42
    for year in range(global_min_year, 2026):
...
print("\nWeekly Aggregated Dataset (2021-2025-W42, zero-filled missing weeks):")
print(df_weekly_filled.head(30))
print(f"\nTotal rows: {len(df_weekly_filled)}")

# Save to Excel
df_weekly_filled.to_excel('./data/demand_prediction_weekly1.xlsx', index=False)
print("\nDataset saved as 'demand_prediction_weekly1.xlsx'")
